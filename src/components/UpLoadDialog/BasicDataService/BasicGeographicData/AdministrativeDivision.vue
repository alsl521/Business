<template>
  <el-dialog v-model="upLoadDialogStore.AdministrativeDivisionDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
    ...
    <div>
      <el-button type="primary" @click="get_AdministrativeDivision">
        加载已有数据
      </el-button>
...
<script setup lang="ts" name="AdministrativeDivisionDialog">
...
const get_AdministrativeDivision = async () => {
  await upLoadDialogStore.get_AdministrativeDivision_Data();
}
...
const submitUpload = async () => {
  ...
  let result: any = await upLoadDialogStore.upLoad_AdministrativeDivision_Data(formData)
  if (result.code == 1) {
    ...
    upLoadDialogStore.AdministrativeDivisionDialogVisible = false
  } else {
    isLoading.value = false;
  }
}
