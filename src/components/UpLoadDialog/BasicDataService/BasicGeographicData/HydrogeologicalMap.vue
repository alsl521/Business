<template>
  <el-dialog v-model="upLoadDialogStore.HydrogeologicalMapDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
    ...
    <el-button type="primary" @click="get_HydrogeologicalMap">
      加载已有数据
    </el-button>
...
<script setup lang="ts" name="HydrogeologicalMapDialogVisible">
...
const get_HydrogeologicalMap = async () => {
  await upLoadDialogStore.get_HydrogeologicalMap_Data();
}
...
const submitUpload = async () => {
  ...
  let result: any = await upLoadDialogStore.upLoad_HydrogeologicalMap_Data(formData)
  if (result.code == 1) {
    ...
    upLoadDialogStore.HydrogeologicalMapDialogVisible= false
  } else {
    isLoading.value = false;
  }
}
