<template>
  <el-dialog v-model="upLoadDialogStore.LandUseDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
    ...
    <el-button type="primary" @click="get_LandUse">
      加载已有数据
    </el-button>
...
<script setup lang="ts" name="LandUseDialogVisible">
...
const get_LandUse = async () => {
  await upLoadDialogStore.get_LandUse_Data();
}
...
const submitUpload = async () => {
  ...
  let result: any = await upLoadDialogStore.upLoad_LandUse_Data(formData)
  if (result.code == 1) {
    ...
    upLoadDialogStore.LandUseDialogVisible= false
  } else {
    isLoading.value = false;
  }
}
