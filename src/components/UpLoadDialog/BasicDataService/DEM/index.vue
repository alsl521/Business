<template>
  <el-dialog v-model="upLoadDialogStore.DEMDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
    ...
    <el-button type="primary" @click="get_DEM">
      加载已有数据
    </el-button>
...
<script setup lang="ts" name="DEMDialogVisible">
...
const get_DEM = async () => {
  await upLoadDialogStore.get_DEM_Data();
}
...
const submitUpload = async () => {
  ...
  let result: any = await upLoadDialogStore.upLoad_DEM_Data(formData)
  if (result.code == 1) {
    ...
    upLoadDialogStore.DEMDialogVisible= false
  } else {
    isLoading.value = false;
  }
}
