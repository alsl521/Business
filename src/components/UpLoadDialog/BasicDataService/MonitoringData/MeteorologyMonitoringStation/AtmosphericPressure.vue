<template>
  <el-dialog v-model="upLoadDialogStore.AtmosphericPressureDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
    ...
    <div v-if="showPages==1">
      <el-button type="primary" @click="getAtmosphericPressure">
        加载已有数据
      </el-button>
    ...
    <div v-if="showPages==2">
      <el-form ref="ruleFormRef" :model="form" label-width="130px" style="margin-top: 20px" :rules="rules">
        <el-form-item>
        ...
        <el-form-item label="大气压强(mbar)" prop="observation_atmosphericpressure">
          <el-select v-model="form.observation_atmosphericpressure" placeholder="请选择大气压强字段">
            <el-option
                v-for="item in file_options"
                :key="item"
                :label="item"
    ...
    <div v-if="showPages==3">
      <el-text size="large" type="primary">大气压强监测点属性表</el-text>
      ...
      <el-button type="primary" style="margin-top: 20px" @click="publishAtmosphericPressure">
        确定
      </el-button>
      ...
      >
        ...
        <el-form :model="Observation_Data_Modification_FormValue" label-width="120px" style="margin-top: 30px">
          ...
          <el-form-item label="大气压强(mbar)">
            <el-input v-model="Observation_Data_Modification_FormValue.atmosphericpressure"/>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="updateLocal">修改</el-button>
          </el-form-item>
...
<script setup lang="ts" name="AtmosphericPressureDialog">
...
//直接获取数据
const getAtmosphericPressure = async () => {
  await upLoadDialogStore.get_AtmosphericPressure_Data();
}
...
const submitUpload = async () => {
  if (formfile.fileList.length == 2 && formfile.fileList[0].raw != formfile.fileList[1].raw) {
    ...
    let result: any = await upLoadDialogStore.upLoad_AtmosphericPressure_Data(formData)
    if (result.code == 1) {
      showPages.value = 2
...
interface RuleForm {
  ...
  observation_atmosphericpressure: string
}
...
const form: any = reactive<RuleForm>({
  ...
  observation_name: '',
  observation_time: '',
  observation_atmosphericpressure: '',
})
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      let result: any = await upLoadDialogStore.create_AtmosphericPressure_Table(form)
...
const handle_Observation_Data_Edit = (index: number, row: any) => {
  ...
  Observation_Data_Modification_FormValue.atmosphericpressure = row.atmosphericpressure
...
const updateLocal = async () => {
  if (Monitoring_Point_Attribute_Modification_Visible.value == true) {
    ...
    let result: any = await upLoadDialogStore.update_AtmosphericPressure_Table(Monitoring_Point_Attribute_Modification_FormValue)
    if (result.code == 1) {
    ...
    observation_Data.value[Observation_Data_Modification_FormValue.index].atmosphericpressure = Observation_Data_Modification_FormValue.atmosphericpressure

    let result: any = await upLoadDialogStore.update_AtmosphericPressure_Table(Observation_Data_Modification_FormValue)
    if (result.code == 1) {
...
const publishAtmosphericPressure = async () => {
  let result: any = await upLoadDialogStore.publish_AtmosphericPressure_Data()

  if (result.code == 1) {
    upLoadDialogStore.AtmosphericPressureDialogVisible = false
  }
}
</script>
