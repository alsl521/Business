<template>
  <el-dialog v-model="upLoadDialogStore.RainfallDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
    ...
    <div v-if="showPages==1">
      <el-button type="primary" @click="getRainfall">
        加载已有数据
      </el-button>
    ...
    <div v-if="showPages==2">
      <el-form ref="ruleFormRef" :model="form" label-width="120px" style="margin-top: 20px" :rules="rules">
        ...
        <el-form-item label="Rainfall(mm/d)" prop="observation_rainfall">
          <el-select v-model="form.observation_rainfall" placeholder="请选择降雨量字段">
            <el-option
                v-for="item in file_options"
                :key="item"
                :label="item"
    ...
    <div v-if="showPages==3">
      <el-text size="large" type="primary">降雨量监测点属性表</el-text>
      ...
      <el-text size="large" type="primary">降雨量观测记录表</el-text>
      <el-table :data="observation_Data" style="width: 100%;margin-top: 10px" border height="200">
        <el-table-column prop="fid" label="Fid"/>
        <el-table-column prop="name" label="Name"/>
        <el-table-column prop="time" label="Time"/>
        <el-table-column prop="rainfall" label="Rainfall(mm/d)"/>
        <el-table-column label="Operations">
          <template #default="scope">
      ...
      <el-button type="primary" style="margin-top: 20px" @click="publishRainfall">
        确定
      </el-button>
      ...
      >
        ...
        <el-form :model="Observation_Data_Modification_FormValue" label-width="120px" style="margin-top: 30px">
          ...
          <el-form-item label="Rainfall(mm/d)">
            <el-input v-model="Observation_Data_Modification_FormValue.Rainfall"/>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="updateLocal">修改</el-button>
          </el-form-item>
...
<script setup lang="ts" name="RainfallDialog">
...
const getRainfall = async () => {
  await upLoadDialogStore.get_Rainfall_Data();
}
...
const submitUpload = async () => {
  if (formfile.fileList.length == 2 && formfile.fileList[0].raw != formfile.fileList[1].raw) {
    ...
    let result: any = await upLoadDialogStore.upLoad_Rainfall_Data(formData)
    if (result.code == 1) {
      showPages.value = 2
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      let result: any = await upLoadDialogStore.create_Rainfall_Table(form)
      console.log(result)
      if (result.code == 1) {
        showPages.value = 3
...
const handle_Observation_Data_Edit = (index: number, row: any) => {
  ...
  Observation_Data_Modification_FormValue.Rainfall = row.Rainfall
...
const updateLocal = async () => {
  if (Monitoring_Point_Attribute_Modification_Visible.value == true) {
    ...
    let result: any = await upLoadDialogStore.update_Rainfall_Table(Monitoring_Point_Attribute_Modification_FormValue)
    if (result.code == 1) {
    ...
    observation_Data.value[Observation_Data_Modification_FormValue.index].Rainfall = Observation_Data_Modification_FormValue.Rainfall

    let result: any = await upLoadDialogStore.update_Rainfall_Table(Observation_Data_Modification_FormValue)
    if (result.code == 1) {
...
const publishRainfall = async () => {
  let result: any = await upLoadDialogStore.publish_Rainfall_Data()
  console.log(result)
  if (result.code == 1) {
    upLoadDialogStore.RainfallDialogVisible = false
  }
}
</script>
