<template>
    <el-dialog v-model="upLoadDialogStore.TemperatureDialogVisible" destroy-on-close>
      <!-- 自定义标题栏 -->
      ...
      <div v-if="showPages==1">
        <el-button type="primary" @click="getTemperature">
          加载已有数据
        </el-button>
      ...
      <div v-if="showPages==3">
        ...
        <el-button type="primary" style="margin-top: 20px" @click="publishTemperature">
          确定
        </el-button>
  ...
  <script setup lang="ts" name="TemperatureDialog">
  ...
  const getTemperature = async () => {
    await upLoadDialogStore.get_Temperature_Data();
  }
  ...
  const submitUpload = async () => {
    if (formfile.fileList.length == 2 && formfile.fileList[0].raw != formfile.fileList[1].raw) {
      ...
      let result: any = await upLoadDialogStore.upLoad_Temperature_Data(formData)
      if (result.code == 1) {
        showPages.value = 2
  ...
  interface RuleForm {
    ...
    observation_maxtemperature: string
    observation_mintemperature: string
  }
  ...
  const form: any = reactive<RuleForm>({
    ...
    observation_name: '',
    observation_time: '',
    observation_maxtemperature: '',
    observation_mintemperature: '',
  })
  ...
  const updateLocal = async () => {
    if (Monitoring_Point_Attribute_Modification_Visible.value == true) {
      ...
      let result: any = await upLoadDialogStore.update_Temperature_Table(Monitoring_Point_Attribute_Modification_FormValue)
      if (result.code == 1) {
  ...
  const publishTemperature= async () => {
    let result: any = await upLoadDialogStore.publish_Temperature()
    console.log(result)
    if (result.code == 1) {
