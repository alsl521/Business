<template>
  <el-dialog v-model="upLoadDialogStore.WaterTemperatureDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
    ...
    <div v-if="showPages==1">
      <el-button type="primary" @click="getWaterTemperature">
        加载已有数据
      </el-button>
    ...
    <div v-if="showPages==2">
      <el-form ref="ruleFormRef" :model="form" label-width="120px" style="margin-top: 20px" :rules="rules">
        ...
        <el-form-item label="WT(℃)" prop="observation_watertemperature">
          <el-select v-model="form.observation_watertemperature" placeholder="请选择水温字段">
            <el-option
                v-for="item in file_options"
                :key="item"
                :label="item"
    ...
    <div v-if="showPages==3">
      <el-text size="large" type="primary">水温监测点属性表</el-text>
      ...
      <el-text size="large" type="primary">水温观测记录表</el-text>
      <el-table :data="observation_Data" style="width: 100%;margin-top: 10px" border height="200">
        <el-table-column prop="fid" label="Fid"/>
        <el-table-column prop="name" label="Name"/>
        <el-table-column prop="time" label="Time"/>
        <el-table-column prop="watertemperature" label="WT(℃)"/>
        <el-table-column label="Operations">
          <template #default="scope">
      ...
      <el-button type="primary" style="margin-top: 20px" @click="publishWaterTemperature">
        确定
      </el-button>
      ...
      >
        ...
        <el-form :model="Observation_Data_Modification_FormValue" label-width="120px" style="margin-top: 30px">
          ...
          <el-form-item label="WT(℃)">
            <el-input v-model="Observation_Data_Modification_FormValue.WaterTemperature"/>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="updateLocal">修改</el-button>
          </el-form-item>
...
<script setup lang="ts" name="WaterTemperatureDialog">
...
const getWaterTemperature = async () => {
  await upLoadDialogStore.get_WaterTemperature_Data();
}
...
const submitUpload = async () => {
  if (formfile.fileList.length == 2 && formfile.fileList[0].raw != formfile.fileList[1].raw) {
    ...
    let result: any = await upLoadDialogStore.upLoad_WaterTemperature_Data(formData)
    if (result.code == 1) {
      showPages.value = 2
...
interface RuleForm {
  ...
  observation_watertemperature: string
}
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      let result: any = await upLoadDialogStore.create_WaterTemperature_Table(form)
      console.log(result)
      if (result.code == 1) {
        showPages.value = 3
...
const handle_Observation_Data_Edit = (index: number, row: any) => {
  ...
  Observation_Data_Modification_FormValue.WaterTemperature = row.WaterTemperature
...
const updateLocal = async () => {
  if (Monitoring_Point_Attribute_Modification_Visible.value == true) {
    ...
    let result: any = await upLoadDialogStore.update_WaterTemperature_Table(Monitoring_Point_Attribute_Modification_FormValue)
    if (result.code == 1) {
    ...
    observation_Data.value[Observation_Data_Modification_FormValue.index].WaterTemperature = Observation_Data_Modification_FormValue.WaterTemperature

    let result: any = await upLoadDialogStore.update_WaterTemperature_Table(Observation_Data_Modification_FormValue)
    if (result.code == 1) {
...
const publishWaterTemperature = async () => {
  let result: any = await upLoadDialogStore.publish_WaterTemperature_Data()
  console.log(result)
  if (result.code == 1) {
    upLoadDialogStore.WaterTemperatureDialogVisible = false
  }
}
</script>
