<template>
  <el-dialog v-model="upLoadDialogStore.WindSpeedAndDirectionDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
    ...
    <div v-if="showPages==1">
      <el-button type="primary" @click="getWindSpeedAndDirection">
        加载已有数据
      </el-button>
    ...
    <div v-if="showPages==3">
      ...
      <el-button type="primary" style="margin-top: 20px" @click="publishWindSpeedAndDirection">
        确定
      </el-button>
...
<script setup lang="ts" name="WindSpeedAndDirectionDialog">
...
//直接获取数据
const getWindSpeedAndDirection = async () => {
  await upLoadDialogStore.get_WindSpeedAndDirection_Data();
}
...
const submitUpload = async () => {
  if (formfile.fileList.length == 2 && formfile.fileList[0].raw != formfile.fileList[1].raw) {
    ...
    let result: any = await upLoadDialogStore.upLoad_WindSpeedAndDirection_Data(formData)
    if (result.code == 1) {
      showPages.value = 2
...
interface RuleForm {
  ...
  observation_windspeed: string
  observation_winddirection: string
}
...
const form: any = reactive<RuleForm>({
  ...
  observation_name: '',
  observation_time: '',
  observation_windspeed: '',
  observation_winddirection: '',
})
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      let result: any = await upLoadDialogStore.create_WindSpeedAndDirection_Table(form)
      console.log(result)
      if (result.code == 1) {
        showPages.value = 3
...
const updateLocal = async () => {
  if (Monitoring_Point_Attribute_Modification_Visible.value == true) {
    ...
    let result: any = await upLoadDialogStore.update_WindSpeedAndDirection_Table(Monitoring_Point_Attribute_Modification_FormValue)
    if (result.code == 1) {
...
const publishWindSpeedAndDirection = async () => {
  let result: any = await upLoadDialogStore.publish_WindSpeedAndDirection_Data()
  console.log(result)
  if (result.code == 1) {
    upLoadDialogStore.WindSpeedAndDirectionDialogVisible = false
  }
}
</script>
