<template>
  <el-dialog v-model="upLoadDialogStore.EFDCParameterDispersionDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
...
<script setup lang="ts" name="EFDCParameterDispersionDialog">
...
const submitUpload = async () => {
  ...
  let result: any = await upLoadDialogStore.upLoad_ModelData_EFDCParameter_Data(formData)
  if (result.code == 1) {
    ...
    upLoadDialogStore.EFDCParameterDispersionDialogVisible = false
  } else {
    isLoading.value = false;
  }
}
