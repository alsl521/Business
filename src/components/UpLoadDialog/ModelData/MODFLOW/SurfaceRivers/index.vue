<template>
  <el-dialog v-model="upLoadDialogStore.MODFLOWSurfaceRiveDialogVisible" destroy-on-close>
    <!-- 自定义标题栏 -->
...
<script setup lang="ts" name="MODFLOWSurfaceRiveDialog">
...
const submitUpload = async () => {
  if (formfile.fileList.length == 2 && formfile.fileList[0].raw != formfile.fileList[1].raw) {
    ...
    let result: any = await upLoadDialogStore.upLoad_MODFLOWSurfaceRive_Data(formData)
    if (result.code == 1) {
      ...
      upLoadDialogStore.MODFLOWSurfaceRiveDialogVisible = false
    }
