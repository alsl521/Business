<!--<template>-->
<!--  <el-dialog v-model="upLoadDialogStore.SWMMHruParameterDispersionDialogVisible" destroy-on-close>-->
<!--    &lt;!&ndash; 自定义标题栏 &ndash;&gt;-->
...
<!--<script setup lang="ts" name="SWMMHruParameterDispersionDialog">-->
...
const submitUpload = async () => {
  ...
  let result: any = await upLoadDialogStore.upLoad_ModelData_SWMM_ParameterDiscrete_HRU_Data(formData)
  if (result.code == 1) {
    ...
    upLoadDialogStore.SWMMHruParameterDispersionDialogVisible = false
  } else {
    isLoading.value = false;
  }
}
